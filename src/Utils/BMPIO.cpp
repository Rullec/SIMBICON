...
{
    return new Image(nrBits / 8, infoHeader->getWidth(),
                     infoHeader->getHeight(), NULL);
}
...
BMPHeader::BMPHeader(Image *img)
{
    ...
    id = ('M' << 8) + 'B';
    fileSize = HEADERSIZE + HEADERINFOSIZE +
               img->getNrBytes() * img->getWidth() * img->getHeight() +
               ((img->getNrBytes() * img->getWidth()) % 4) *
                   img->getHeight(); //this is the padding
...
BMPInfoHeader::BMPInfoHeader(FILE *fp)
{
    ...
    if (nrBits == 8)
    {
        pallete = (unsigned char *)malloc(4 * sizeof(unsigned char) *
                                          ((nrColours == 0) ? 256 : nrColours));
        fread(pallete, sizeof(unsigned char),
              4 * ((nrColours == 0) ? 256 : nrColours), fp);
    }
    /* done populating the info header from a file */
}
