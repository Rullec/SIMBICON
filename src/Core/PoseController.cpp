...
*/
Vector3d PoseController::computePDTorque(const Quaternion &qRel,
                                         const Quaternion &qRelD,
                                         const Vector3d &wRel,
...
*/
void PoseController::scaleAndLimitTorque(Vector3d *torque,
                                         ControlParams *cParams,
                                         const Quaternion &qToChild)
{
    ...
    for (int i = 0; i < jointCount; i++)
    {
        if (controlParams[i].controlled == true)
        {
            if (controlParams[i].relToCharFrame == false)
            {
                ...
                torques[i] = computePDTorque(
                    qRel, rs.getJointRelativeOrientation(i), wRel,
                    rs.getJointRelativeAngVelocity(i), &controlParams[i]);
                //the torque is expressed in parent coordinates, so we need to convert it to world coords now
                torques[i] =
                    character->getJoint(i)->getParent()->getWorldCoordinates(
                        torques[i]);
            }
            else
            {
                ...
                torques[i] = computePDTorque(
                    childRB->getOrientation(),
                    controlParams[i].charFrame *
                        rs.getJointRelativeOrientation(i),
                    childRB->getAngularVelocity(),
                    rs.getJointRelativeAngVelocity(i), &controlParams[i]);
            }
        }
        else
...
void PoseController::parseGainLine(char *line)
{
    ...
    if (nrParams == 2)
    {
        ...
        throwError("To specify the gains, you need: 'joint name Kp Kd Tmax "
...
void PoseController::readGains(FILE *f)
{
    ...
    throwError("Incorrect controller input file: No \'/KpKdMaxT\' found",
               buffer);
}
...
void PoseController::writeGains(FILE *f)
{
    ...
    for (uint jIndex = 0; jIndex < controlParams.size(); ++jIndex)
    {
        ...
        fprintf(f, "    %s\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", jName,
                controlParams[jIndex].kp, controlParams[jIndex].kd,
                controlParams[jIndex].maxAbsTorque,
                controlParams[jIndex].scale.x, controlParams[jIndex].scale.y,
