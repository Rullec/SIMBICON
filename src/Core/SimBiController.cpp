...
{
    if (b == NULL)
        throwError("Cannot create a SIMBICON controller if there is no "
                   "associated biped!!");
    ...
    if (lHip->getParent() != rHip->getParent() || lHip->getParent() != root)
        throwError("The biped's hips must have a common parent, which should "
                   "be the root of the figure!");
...
*/
Vector3d SimBiController::getForceOn(RigidBody *rb,
...
bool SimBiController::isSwingFoot(RigidBody *rb)
{
    ...
    if (rb == swingFoot)
        return true;
    //and against the toes
    for (uint j = 0; j < ((ArticulatedRigidBody *)swingFoot)->cJoints.size();
         j++)
...
bool SimBiController::isStanceFoot(RigidBody *rb)
{
    ...
    if (rb == stanceFoot)
        return true;
    //and against the toes
    for (uint j = 0; j < ((ArticulatedRigidBody *)stanceFoot)->cJoints.size();
         j++)
                                         ...
                                         DynamicArray<ContactPoint> *cfs)
{
    ...
    {
        fNet +=
            getForceOn(((ArticulatedRigidBody *)foot)->cJoints[i]->child, cfs);
    }
    return fNet;
}
...
int SimBiController::advanceInTime(double dt, DynamicArray<ContactPoint> *cfs)
{
    ...
    for (uint i = 0; i < cfs->size(); i++)
    {
        ...
        if ((*cfs)[i].rb1->isArticulated() == false &&
            (*cfs)[i].rb2->isArticulated() == false)
            continue;
    ...
    if (states[FSMStateIndex]->needTransition(
            phi,
            fabs(getForceOnFoot(swingFoot, cfs).dotProductWith(SimGlobals::up)),
            fabs(getForceOnFoot(stanceFoot, cfs)
                     .dotProductWith(SimGlobals::up))))
    {
...
*/
double
SimBiController::getStanceFootWeightRatio(DynamicArray<ContactPoint> *cfs)
{
    Vector3d stanceFootForce = getForceOnFoot(stanceFoot, cfs);
    Vector3d swingFootForce = getForceOnFoot(swingFoot, cfs);
    double totalYForce =
        (stanceFootForce + swingFootForce).dotProductWith(SimGlobals::up);
...
*/
void SimBiController::updateTrackingPose(DynamicArray<double> &trackingPose,
                                         double phiToUse)
{
    if (phiToUse < 0)
        phiToUse = phi;
    ...
    for (int i = 0; i < curState->getTrajectoryCount(); i++)
    {
        ...
        if (curState->sTraj[i]->relToCharFrame == true ||
            jIndex == swingHipIndex)
            controlParams[jIndex].relToCharFrame = true;
        Vector3d d0, v0;
        computeD0(phiToUse, &d0);
        computeV0(phiToUse, &v0);
        Quaternion newOrientation = curState->sTraj[i]->evaluateTrajectory(
            this, character->getJoint(jIndex), stance, phiToUse, d - d0,
            v - v0);
        if (jIndex == -1)
        {
    ...
    for (int i = 0; i < jointCount; i++)
    {
        if (controlParams[i].relToCharFrame)
        {
            ...
            while (parent != root)
            {
                ...
                parent = j->getParent();
                temp = debugRS.getJointRelativeOrientation(
                           character->getJointIndex(j->name)) *
                       temp;
            }

            temp = qRootD * temp;
            temp = temp.getComplexConjugate() *
                   debugRS.getJointRelativeOrientation(i);
...
void SimBiController::computeTorques(DynamicArray<ContactPoint> *cfs)
{
    ...
    for (int i = 0; i < curState->getTrajectoryCount(); i++)
    {
        ...
        computeD0(phiToUse, &d0);
        computeV0(phiToUse, &v0);
        newOrientation = curState->sTraj[i]->evaluateTrajectory(
            this, character->getJoint(jIndex), stance, phiToUse, d - d0,
            v - v0);
        ...
        if (jIndex == -1)
        {
            ...
            rootControlParams.strength =
                curState->sTraj[i]->evaluateStrength(phiToUse);
        }
        else
        {
            if (curState->sTraj[i]->relToCharFrame == true ||
    ...
    computeHipTorques(qRootD, poseRS.getJointRelativeOrientation(swingHipIndex),
                      stanceHipToSwingHipRatio);
...
*/
void SimBiController::computeHipTorques(const Quaternion &qRootD,
                                        ...
                                        double stanceHipToSwingHipRatio)
{
    ...
    {
        double wRely =
            root->getAngularVelocity().y -
            character->joints[stanceHipIndex]->child->getAngularVelocity().y;
        if (fabs(wRely) > stanceHipMaxVelocity)
            wRely = stanceHipMaxVelocity * SGN(wRely);
...
void SimBiController::updateDAndV()
{
    ...
    v = characterFrame.getComplexConjugate().rotate(
        character->getCOMVelocity());
}
...
void SimBiController::writeToFile(char *fileName, char *stateFileName)
{
    ...
    if (stanceHipDamping > 0)
    {
        fprintf(f, "%s %lf\n", getConLineString(CON_STANCE_HIP_DAMPING),
                stanceHipDamping);
        fprintf(f, "%s %lf\n", getConLineString(CON_STANCE_HIP_MAX_VELOCITY),
    ...
    if (stateFileName == NULL)
        fprintf(f, "%s %s\n", getConLineString(CON_CHARACTER_STATE),
                initialBipState);
    else
        fprintf(f, "%s %s\n", getConLineString(CON_CHARACTER_STATE),
                stateFileName);
...
void SimBiController::parseGainLine(char *line)
{
    ...
    if (sscanf(line, "%s", jName) != 1)
        throwError("To specify the PD gains, you need tp specify the "
                   "jointName/root --> \'%s\'",
                   line);
    if (strcmp(jName, "root") == 0)
    {
        if (sscanf(line, "%s %lf %lf %lf %lf %lf %lf\n", jName, &kp, &kd, &tMax,
                   &scX, &scY, &scZ) != 7)
            throwError("To specify the PD gains, you need: 'jointName Kp Kd "
                       "Tmax scaleX scaleY scaleZ'! --> \'%s\'",
                       line);
    ...
    while (!feof(f))
    {
        ...
        case CON_STARTING_STANCE:
            ...
            else if (strncmp(trim(line), "right", 5) == 0)
            {
                ...
                throwError("When using the \'reverseTargetOnStance\' keyword, "
