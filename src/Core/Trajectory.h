...
public:
	...
	*/
    double getKnotPosition(int i) { return tValues[i]; }
	...
	*/
    int getKnotCount() { return tValues.size(); }
    ...
    void simplify_catmull_rom(double maxError, int nbSamples = 100)
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < nbSamples; ++i)
            {
                ...
                double error = abs(result.evaluate_catmull_rom(time) -
                                   evaluate_catmull_rom(time));
                if (error > currError)
                {
                    currError = error;
                    currErrorTime = time;
