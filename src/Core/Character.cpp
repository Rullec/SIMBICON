...
Character::Character(ArticulatedFigure *ch)
{
    if (ch == NULL)
        throwError(
            "Can't build a character without an articulated figure!!!\n");
    this->af = ch;
...
{
    *wRel = joints[i]->child->state.angularVelocity -
            joints[i]->parent->state.angularVelocity;
...
void Character::setState(DynamicArray<double> *state, int start)
{
    ...
    for (uint j = 0; j < joints.size(); j++)
    {
        ...
        joints[j]->child->state.orientation =
            joints[j]->parent->state.orientation * qRel;
        //and the proper angular velocity
        joints[j]->child->state.angularVelocity =
            joints[j]->parent->state.angularVelocity + wRel;
...
Quaternion mirrorOrientation(const Quaternion &q)
{
    ...
    Quaternion qSagittal = q.getComplexConjugate()
                               .decomposeRotation(Vector3d(1, 0, 0))
                               .getComplexConjugate();
...
*/
void Character::reverseStanceOfStateArray(DynamicArray<double> *state,
                                          int start)
{
    ...
    {
        //orientations
        chS.setJointRelativeOrientation(
            mirrorOrientation(chS.getJointRelativeOrientation(i)), i);
        ...
        chS.setJointRelativeAngVelocity(
            elemWiseMultiply(chS.getJointRelativeAngVelocity(i),
                             Vector3d(1, -1, -1)),
            i);
    }
    ...
    chS.setAngularVelocity(
        elemWiseMultiply(chS.getAngularVelocity(), Vector3d(1, -1, -1)));
...
*/
int Character::getJointCount() { return joints.size(); }

/**
	This method is used to return the articulated figure of the character.
*/
ArticulatedFigure *Character::getAF() { return af; }
...
*/
void Character::setHeading(Quaternion heading, DynamicArray<double> *state,
                           int start)
{
    ReducedCharacterState chS(state, start);
...
void Character::setHeading(double val)
{
    DynamicArray<double> state;
    getState(&state);
    setHeading(Quaternion::getRotationQuaternion(val, Vector3d(0, 1, 0)),
               &state);
    setState(&state);
}
...
*/
void Character::readReducedStateFromFile(char *fName,
                                         DynamicArray<double> *state)
{
    if (fName == NULL)
    ...
    //now set the heading...
    setHeading(Quaternion::getRotationQuaternion(heading, SimGlobals::up),
               state, start);
...
void Character::saveReducedStateToFile(char *fName, DynamicArray<double> &state)
{
    ...
    for (uint i = 0; i < joints.size(); i++)
    {
        fprintf(fp, "# %s\n", joints[i]->name);
        fprintf(fp, "%lf %lf %lf %lf\n", state[13 + 7 * i + 0],
                state[13 + 7 * i + 1], state[13 + 7 * i + 2],
                state[13 + 7 * i + 3]);
        fprintf(fp, "%lf %lf %lf\n", state[13 + 7 * i + 4],
                state[13 + 7 * i + 5], state[13 + 7 * i + 6]);
        fprintf(fp, "\n");
    }
    fclose(fp);
}
