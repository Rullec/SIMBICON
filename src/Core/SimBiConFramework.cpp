...
SimBiConFramework::~SimBiConFramework(void) { delete con; }
...
/**
	...
	otherwise.
*/
bool SimBiConFramework::advanceInTime(double dt, bool applyControl,
                                      bool recomputeTorques,
                                      bool advanceWorldInTime)
{
    if (applyControl == false)
        con->resetTorques();
    else if (recomputeTorques == true)
    ...
    if (newFSMState)
    {
        ...
        lastStepTaken = con->getCharacterFrame().getComplexConjugate().rotate(
            lastStepTaken);
        lastFootPos = con->getStanceFootPos();
    }
