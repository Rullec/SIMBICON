...
void HingeJoint::readAxes(char *axes)
{
    if (sscanf(axes, "%lf %lf %lf", &a.x, &a.y, &a.z) != 3)
        throwError("Hinge joints require the rotation axis to be provided as a "
                   "parameter!");
...
void HingeJoint::readJointLimits(char *limits)
{
    if (sscanf(limits, "%lf %lf", &minAngle, &maxAngle) != 2)
        throwError("Two parameters are needed to specify joint limits for a "
                   "hinge joint!");
    useJointLimits = true;
}
...
*/
    ...
    child->state.orientation =
        parent->state.orientation * Quaternion::getRotationQuaternion(ang, a);
}
