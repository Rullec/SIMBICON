...
void AbstractRBEngine::setState(DynamicArray<double> *state, int start)
{
    ...
    {
        objects[j]->state.position =
            Point3d((*state)[i + 0], (*state)[i + 1], (*state)[i + 2]);
        i += 3;
        objects[j]->state.orientation = Quaternion(
            (*state)[i + 0], (*state)[i + 1], (*state)[i + 2], (*state)[i + 3]);
        i += 4;
        objects[j]->state.velocity =
            Vector3d((*state)[i + 0], (*state)[i + 1], (*state)[i + 2]);
        i += 3;
        objects[j]->state.angularVelocity =
            Vector3d((*state)[i + 0], (*state)[i + 1], (*state)[i + 2]);
        i += 3;
