...
void Joint::computeRelativeOrientation(Quaternion &qRel)
{
    ...
    qRel = parent->state.orientation.getComplexConjugate() *
           child->state.orientation;
}
...
*/
void Joint::fixJointConstraints(bool fixOrientations, bool fixVelocities,
                                bool recursive)
{
    if (!child)
        return;
    ...
    if (parent)
    {
        ...
        Vector3d rc =
            child->getWorldCoordinates(Vector3d(cJPos, Point3d(0, 0, 0)));
        //and the vector rp that represents the same quanity but for the parent
        Vector3d rp =
            parent->getWorldCoordinates(Vector3d(pJPos, Point3d(0, 0, 0)));
        ...
        if (fixVelocities)
        {
            ...
            Vector3d r =
                Vector3d(parent->state.position, child->state.position);
            Vector3d d = Vector3d(child->getWorldCoordinates(cJPos),
                                  child->state.position);
            Vector3d vRel = parent->state.angularVelocity.crossProductWith(r) +
                            wRel.crossProductWith(d);
    ...
    if (recursive)
        for (uint i = 0; i < child->cJoints.size(); i++)
            child->cJoints[i]->fixJointConstraints(fixOrientations,
...
void Joint::loadFromFile(FILE *f, AbstractRBEngine *world)
{
    ...
    while (!feof(f))
    {
        ...
        case RB_PARENT:
            ...
            if (parent == NULL)
                throwError("The articulated rigid body \'%s\' cannot be found!",
                           tempName);
            break;
        case RB_CHILD:
            sscanf(line, "%s", tempName);
        ...
        case RB_END_JOINT:
            ...
            if (child == NULL)
                throwError("A joint has been found that does not have a child "
                           "rigid body");
            if (parent == NULL)
                throwError("A parent has been found that does not have a child "
                           "rigid body");
            if (child->pJoint != NULL)
                throwError("The child body \'%s\' already has a parent.",
                           child->name);
        ...
        case RB_NOT_IMPORTANT:
            ...
            break;
        default:
            throwError("Incorrect articulated body input file: \'%s\' - "
                       "unexpected line.",
                       buffer);
        }
    }
