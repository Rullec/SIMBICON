...
ODEWorld::ODEWorld() : AbstractRBEngine()
{
    ...
    dWorldSetContactSurfaceLayer(
        worldID,
        0.001); // the ammount of interpenetration allowed between objects
    dWorldSetContactMaxCorrectingVel(
        worldID, 1.0); // maximum velocity that contacts are allowed to generate
...
void ODEWorld::setODEStateFromRB(int i)
{
    ...
    dBodySetPosition(odeToRbs[i].id, odeToRbs[i].rb->state.position.x,
                     odeToRbs[i].rb->state.position.y,
                     odeToRbs[i].rb->state.position.z);
    dBodySetQuaternion(odeToRbs[i].id, tempQ);
    dBodySetLinearVel(odeToRbs[i].id, odeToRbs[i].rb->state.velocity.x,
                      odeToRbs[i].rb->state.velocity.y,
                      odeToRbs[i].rb->state.velocity.z);
    dBodySetAngularVel(odeToRbs[i].id, odeToRbs[i].rb->state.angularVelocity.x,
                       odeToRbs[i].rb->state.angularVelocity.y,
                       odeToRbs[i].rb->state.angularVelocity.z);
}
...
dGeomID ODEWorld::getCapsuleGeom(CapsuleCDP *c)
{
    ...
    Quaternion relOrientation =
        Quaternion::getRotationQuaternion(rotAngle, axis);
...
void ODEWorld::setupODEHingeJoint(HingeJoint *hj)
{
    ...
    dJointAttach(j, odeToRbs[(int)(hj->child->id)].id,
                 odeToRbs[(int)(hj->parent->id)].id);
...
void ODEWorld::setupODEBallAndSocketJoint(BallInSocketJoint *basj)
{
    ...
    dJointAttach(aMotor, odeToRbs[(int)(basj->parent->id)].id,
                 odeToRbs[(int)(basj->child->id)].id);
...
void ODEWorld::createODECollisionPrimitives(RigidBody *body)
{
    ...
    for (uint j = 0; j < body->cdps.size(); j++)
    {
        ...
        case PLANE_CDP:
            ...
            break;
        default:
            throwError("Ooppps... No collision detection primitive was created "
                       "rb: %s, cdp: %d",
                       body->name, j);
        }
        ...
        {
            dGeomSetPosition(t, body->state.position.x, body->state.position.y,
    ...
    for (uint i = index; i < objects.size(); i++)
    {
        ...
        if (objects[i]->isLocked() == false)
        {
            ...
            m.setParameters(odeToRbs[i].rb->getMass(), 0, 0, 0,
                            principalMoments.x, principalMoments.y,
                            principalMoments.z, 0, 0, 0);
...
void ODEWorld::processCollisions(dGeomID o1, dGeomID o2)
{
    ...
    int num_contacts =
        dCollide(o1, o2, maxContactCount, &(cps[0].geom), sizeof(dContact));
    ...
    {
        cps[i].surface.mode = dContactSoftERP | dContactSoftCFM |
    ...
    for (int i = 0; i < num_contacts; i++)
    {
        ...
        if (jointFeedbackCount >= MAX_CONTACT_FEEDBACK)
            tprintf("Warning: too many contacts are established. Some of them "
        ...
        else
        {
            ...
            contactPoints[jointFeedbackCount].cp = Point3d(
                cps[i].geom.pos[0], cps[i].geom.pos[1], cps[i].geom.pos[2]);
...
void ODEWorld::advanceInTime(double deltaT)
{
    ...
    {
        contactPoints[i].f =
            Vector3d(jointFeedback[i].f1[0], jointFeedback[i].f1[1],
                     jointFeedback[i].f1[2]);
        //make sure that the force always points away from the static objects
        if (contactPoints[i].rb1->isLocked() &&
            !contactPoints[i].rb2->isLocked())
        {
...
*/
void ODEWorld::applyRelForceTo(RigidBody *b, const Vector3d &f,
                               const Point3d &p)
{
    if (!b)
        return;
