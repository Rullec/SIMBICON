...
BallInSocketJoint::BallInSocketJoint(char *axes) { readAxes(axes); }
...
{
    if (sscanf(axes, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &swingAxis1.x,
               &swingAxis1.y, &swingAxis1.z, &swingAxis2.x, &swingAxis2.y,
               &swingAxis2.z, &twistAxis.x, &twistAxis.y, &twistAxis.z) != 9)
    {
        if (sscanf(axes, "%lf %lf %lf %lf %lf %lf", &swingAxis1.x,
                   &swingAxis1.y, &swingAxis1.z, &twistAxis.x, &twistAxis.y,
                   &twistAxis.z) != 6)
        {
            throwError("Ball in socket joints require two or three axis to be "
                       "specified!");
        }
        else
...
{
    int n = sscanf(limits, "%lf %lf %lf %lf %lf %lf", &minSwingAngle1,
                   &maxSwingAngle1, &minSwingAngle2, &maxSwingAngle2,
                   &minTwistAngle, &maxTwistAngle);
    if (n != 6)
        throwError("Ball in socket joints require 6 joint limit parameters "
                   "(min/max angle for the three axis)!");
