...
{
    Vector3d v = getLocalCoordinates(Vector3d(globalPoint)) -
                 getLocalCoordinates(Vector3d(this->state.position));
...
Vector3d RigidBody::getAbsoluteVelocityForLocalPoint(const Point3d &localPoint)
{
    ...
    return state.angularVelocity.crossProductWith(getWorldCoordinates(r)) +
           state.velocity;
}
...
*/
Vector3d
RigidBody::getAbsoluteVelocityForGlobalPoint(const Point3d &globalPoint)
{
    //we need to compute the vector r, from the origin of the body to the point of interest
    ...
    while (!feof(f))
    {
        ...
        case RB_MASS:
            if (sscanf(line, "%lf", &t) != 1)
                throwError("Incorrect rigid body input file - a mass needs to "
                           "be specified if the 'mass' keyword is used.");
            this->props.setMass(t);
            break;
        case RB_MOI:
            if (sscanf(line, "%lf %lf %lf", &t1, &t2, &t3) != 3)
                throwError("Incorrect rigid body input file - the three "
                           "principal moments of inertia need to be specified "
                           "if the 'moi' keyword is used.");
            if (t1 <= 0 || t2 <= 0 || t3 <= 0)
                throwError(
                    "Incorrect values for the principal moments of inertia.");
        ...
        case RB_COLOUR:
            if (sscanf(line, "%lf %lf %lf %lf", &r, &g, &b, &a) != 4)
                throwError("Incorrect rigid body input file - colour parameter "
                           "expects 4 arguments (colour %s)\n",
                           line);
            if (meshes.size() > 0)
        ...
        case RB_SPHERE:
            if (sscanf(line, "%lf %lf %lf %lf", &p1.x, &p1.y, &p1.z, &r) != 4)
                throwError("Incorrect rigid body input file - 4 arguments are "
                           "required to specify a sphere collision detection "
                           "primitive\n",
                           line);
            cdps.push_back(new SphereCDP(this, p1, r));
            break;
        case RB_CAPSULE:
            if (sscanf(line, "%lf %lf %lf %lf %lf %lf %lf", &p1.x, &p1.y, &p1.z,
                       &p2.x, &p2.y, &p2.z, &r) != 7)
                throwError("Incorrect rigid body input file - 7 arguments are "
                           "required to specify a capsule collision detection "
                           "primitive\n",
                           line);
            cdps.push_back(new CapsuleCDP(this, p1, p2, r));
            break;
        case RB_BOX:
            if (sscanf(line, "%lf %lf %lf %lf %lf %lf", &p1.x, &p1.y, &p1.z,
                       &p2.x, &p2.y, &p2.z) != 6)
                throwError(
                    "Incorrect rigid body input file - 6 arguments are "
                    "required to specify a box collision detection primitive\n",
                    line);
            cdps.push_back(new BoxCDP(this, p1, p2));
            break;
        case RB_PLANE:
            if (sscanf(line, "%lf %lf %lf %lf %lf %lf", &n.x, &n.y, &n.z, &p1.x,
                       &p1.y, &p1.z) != 6)
                throwError("Incorrect rigid body input file - 6 arguments are "
                           "required to specify a plane collision detection "
                           "primitive\n",
                           line);
        ...
        case RB_LOCKED:
            ...
            break;
        case RB_POSITION:
            if (sscanf(line, "%lf %lf %lf", &state.position.x,
                       &state.position.y, &state.position.z) != 3)
                throwError("Incorrect rigid body input file - 3 arguments are "
                           "required to specify the world coordinates position "
                           "of a rigid body\n",
                           line);
            break;
        case RB_ORIENTATION:
            if (sscanf(line, "%lf %lf %lf %lf", &t, &t1, &t2, &t3) != 4)
                throwError("Incorrect rigid body input file - 4 arguments are "
                           "required to specify the world coordinates "
                           "orientation of a rigid body\n",
                           line);
            state.orientation = Quaternion::getRotationQuaternion(
                                    t, Vector3d(t1, t2, t3).toUnit()) *
                                state.orientation;
            break;
        case RB_VELOCITY:
            if (sscanf(line, "%lf %lf %lf", &state.velocity.x,
                       &state.velocity.y, &state.velocity.z) != 3)
                throwError("Incorrect rigid body input file - 3 arguments are "
                           "required to specify the world coordinates velocity "
                           "of a rigid body\n",
                           line);
            break;
        case RB_ANGULAR_VELOCITY:
            if (sscanf(line, "%lf %lf %lf", &state.angularVelocity.x,
                       &state.angularVelocity.y, &state.angularVelocity.z) != 3)
                throwError("Incorrect rigid body input file - 3 arguments are "
                           "required to specify the world coordinates angular "
                           "velocity of a rigid body\n",
                           line);
            break;
        case RB_FRICTION_COEFF:
            if (sscanf(line, "%lf", &props.mu) != 1)
                throwError("Incorrect rigid body input file - Expecting a "
                           "value for the friction coefficient");
            if (props.mu < 0)
                throwError("Incorrect rigid body input file - Friction "
                           "coefficient should be >= 0");
            break;
        case RB_RESTITUTION_COEFF:
            if (sscanf(line, "%lf", &props.epsilon) != 1)
                throwError("Incorrect rigid body input file - Expecting a "
                           "value for the restitution coefficient");
            if (props.epsilon < 0 || props.epsilon > 1)
                throwError("Incorrect rigid body input file - restitution "
                           "coefficient should be between 0 and 1");
            break;
        case RB_ODE_GROUND_COEFFS:
            if (sscanf(line, "%lf %lf", &t1, &t2) != 2)
                throwError("Two parameters need to be provided for the ODE "
                           "ground parameter settings");
        ...
        default:
            throwError(
                "Incorrect rigid body input file: \'%s\' - unexpected line.",
                buffer);
        }
    }
