...
{
    if (sscanf(axes, "%lf %lf %lf %lf %lf %lf", &a.x, &a.y, &a.z, &b.x, &b.y,
               &b.z) != 6)
        throwError("Universal joints require two rotation axes to be provided "
                   "as parameters!");
...
{
    if (sscanf(limits, "%lf %lf %lf %lf", &minAngleA, &maxAngleA, &minAngleB,
               &maxAngleB) != 4)
        throwError("Universal joints require 4 joint limites (minAngleA, "
                   "maxAngleA, minAngleB, maxAngleB)!");
    else
        useJointLimits = true;
}
...
void UniversalJoint::fixAngularConstraint(const Quaternion &qRel)
{
    ...
    child->state.orientation = parent->state.orientation *
                               Quaternion::getRotationQuaternion(angA, a) *
                               tmpQ2;
}
