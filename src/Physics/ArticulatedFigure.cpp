...
*/
void ArticulatedFigure::fixJointConstraints(bool fixOrientations,
                                            bool fixVelocities)
{
    if (!root)
        return;
    ...
    for (uint i = 0; i < root->cJoints.size(); i++)
        root->cJoints[i]->fixJointConstraints(fixOrientations, fixVelocities,
                                              true);
}
...
*/
double ArticulatedFigure::getMass() { return mass; }
...
void ArticulatedFigure::loadFromFile(FILE *f, AbstractRBEngine *world)
{
    ...
    while (!feof(f))
    {
        ...
        case RB_END_ARTICULATED_FIGURE:
            ...
            if (root->pJoint != NULL)
                throwError("The root of the articulated figure is not allowed "
                           "to have a parent!");
            return; //and... done
            break;
