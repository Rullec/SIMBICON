...
void CurveEditor::draw()
{
    ...
    int nbDigit = 0;
    if (vTickSize < 1)
        nbDigit = (int)((-log10(vTickSize)) + 0.5);
    for (double pos = vTickStart; pos <= maxX; pos += vTickSize)
    {
    ...
    nbDigit = 0;
    if (hTickSize < 1)
        nbDigit = (int)((-log10(hTickSize)) + 0.5);
    for (double val = hTickStart; val <= maxY; val += hTickSize)
    {
...
*/
bool CurveEditor::onMouseEvent(int eventType, int button, int mouseX,
                               int mouseY)
{
    ...
    if (editorX < 0 || editorX >= editorSizeX || editorY < 0 ||
        editorY >= editorSizeY)
        return false;
        ...
        if (eventType == MOUSE_DOWN)
        {
            ...
            if (button == MOUSE_RBUTTON)
            {
                ...
                if (glutGetModifiers() & GLUT_ACTIVE_SHIFT)
                {
                    ...
                    {
                        double distX =
                            viewportXToPixel(trajectory->getKnotPosition(i)) -
                            editorX;
                        double distY =
                            viewportYToPixel(trajectory->getKnotValue(i)) -
                            editorY;
            ...
            else if (button == MOUSE_LBUTTON)
            {
                ...
                if (glutGetModifiers() & GLUT_ACTIVE_SHIFT)
                {
                    if (!trajectory)
                        return false;
                    trajectory->addKnot(pixelXToViewport(editorX),
                                        pixelYToViewport(editorY));
                    return true;
                }
    ...
    if (pointHeldIdx >= 0)
    {
        ...
        if (!trajectory || pointHeldIdx >= trajectory->getKnotCount() ||
        ...
        else
        {
            if (newPos <= trajectory->getKnotPosition(pointHeldIdx - 1))
                newPos =
                    trajectory->getKnotPosition(pointHeldIdx - 1) + 0.00001;
        }
