 ...
 */
void ControllerEditor::loadFramework() { loadFramework(-1); }
    ...
    if (shadowMode == false)
    {
        ...
        if (Globals::drawContactForces)
        {
            ...
            DynamicArray<ContactPoint> *cfs =
                conF->getWorld()->getContactForces();
...
{
    if (nextControlShot >= maxControlShot + 1)
        return;
    nextControlShot++;
    clearEditedCurves();
    delete conF;
...
void ControllerEditor::stepTaken()
{
    ...
    if (Globals::updateDVTraj)
    {
        ...
        if (conF)
        {
            ...
            state->updateDVTrajectories(NULL, NULL, dTrajX, dTrajZ, vTrajX,
                                        vTrajZ);
 ...
 */
bool ControllerEditor::onMouseEvent(int eventType, int button, int mouseX,
                                    int mouseY)
{
    ...
    {
        for (uint i = 0; i < curveEditors.size(); ++i)
            if (curveEditors[i]->onMouseEvent(eventType, button, mouseX,
                                              mouseY))
            {
                return true;
            }
 ...
 */
void ControllerEditor::init() { InteractiveWorld::init(); }
...
Point3d ControllerEditor::getCameraTarget()
{
    ...
    return Point3d(ch->getRoot()->getCMPosition().x, 1,
                   ch->getRoot()->getCMPosition().z);
}
...
void ControllerEditor::processTask()
{
    ...
    while (simulationTime / maxRunningTime <
           Globals::animationTimeToRealTimeRatio)
    {
        ...
        double signChange =
            (conF->getController()->getStance() == RIGHT_STANCE) ? -1 : 1;
        ...
        avgSpeed +=
            conF->getCharacter()
                ->getHeading()
                .getComplexConjugate()
                .rotate(conF->getCharacter()->getRoot()->getCMVelocity())
                .z;
        timesVelSampled++;
...
int updateTargetPose(ClientData clientData, Tcl_Interp *interp, int argc,
                     CONST84 char **argv)
{
    ControllerEditor *obj = (ControllerEditor *)clientData;
    return TCL_OK;
...
int controllerUndo(ClientData clientData, Tcl_Interp *interp, int argc,
                   CONST84 char **argv)
{
...
// Following are wrappers for TCL functions that can access the object
int controllerRedo(ClientData clientData, Tcl_Interp *interp, int argc,
                   CONST84 char **argv)
{
...
// Following are wrappers for TCL functions that can access the object
int getStateNames(ClientData clientData, Tcl_Interp *interp, int argc,
                  CONST84 char **argv)
{
...
int getTrajectoryNames(ClientData clientData, Tcl_Interp *interp, int argc,
                       CONST84 char **argv)
{
...
int getComponentNames(ClientData clientData, Tcl_Interp *interp, int argc,
                      CONST84 char **argv)
{
...
void ControllerEditor::registerTclFunctions()
{
    ...
    Tcl_CreateCommand(Globals::tclInterpreter, "getStateNames", getStateNames,
                      (ClientData)this, (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "getTrajectoryNames",
                      getTrajectoryNames, (ClientData)this,
                      (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "getComponentNames",
                      getComponentNames, (ClientData)this,
                      (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "trajectoryToEdit",
                      trajectoryToEdit, (ClientData)this,
                      (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "updateTargetPose",
                      updateTargetPose, (ClientData)this,
                      (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "controllerUndo", controllerUndo,
                      (ClientData)this, (Tcl_CmdDeleteProc *)NULL);

    Tcl_CreateCommand(Globals::tclInterpreter, "controllerRedo", controllerRedo,
                      (ClientData)this, (Tcl_CmdDeleteProc *)NULL);
}
 ...
 */
void ControllerEditor::getDodgeBallPosAndVel(double x, double y,
                                             double strength, Point3d *pos,
                                             Vector3d *vel)
{
    vel->x = x;
    vel->y = 0;
