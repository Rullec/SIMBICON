...
*/
    ...
    if (Globals::drawFPS)
        drawFPSandPerf(fpsTimer.timeEllapsed(),
                       timeSpentProcessing / (1 / Globals::desiredFrameRate));
 ...
 */
void GLWindow::drawFPSandPerf(double timeSinceLastUpdate,
...
int GLWindow::onMouseEvent(int mEvent, int button, int x, int y)
{
    ...
    if (mEvent == MOUSE_DOWN || mEvent == MOUSE_UP ||
        mEvent == MOUSE_DRAG /* || mEvent == MOUSE_MOVE*/)
    {
        ...
        if (processed == false)
        {
            if (mEvent == MOUSE_DRAG)
            {
                ...
                case MOUSE_RBUTTON:
                    ...
                    break;
                case MOUSE_LBUTTON:
                    camera.rotations -= Vector3d((-oldMouseY + y) / 100.0,
                                                 (-oldMouseX + x) / 100.0, 0);
                    break;
                case MOUSE_MBUTTON:
