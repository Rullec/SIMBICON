...
InteractionInterface::InteractionInterface(char *bgName, int posX, int posY,
                                           int size)
{
    this->posX = posX;
    this->posY = posY;
...
void InteractionInterface::drawInterface()
{
    ...
    if (t.timeEllapsed() < 1)
    {
        ...
        Point3d org = Point3d(0.5, 0.5, 0) + inputVector +
                      inputVector.unit() * unusedCenter;
...
*/
bool InteractionInterface::handleMouseEvent(int mouseX, int mouseY,
                                            Vector3d *result)
{
    if (mouseX > posX && mouseX < posX + size && mouseY > posY &&
        mouseY < posY + size)
    {
        double rad = size / 2;
