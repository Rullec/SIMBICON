...
bool PointPrimitive::intersectSegments(Point3d &start1, Point3d &end1,
                                       Point3d &start2, Point3d &end2,
    ...
    for (int j = size + 1; --j;)
    {
        ...
        if (yFlag0 != yFlag1)
        {
            ...
            if (((nextPoint->y - point.y) * (currPoint->x - nextPoint->x) >=
                 (nextPoint->x - point.x) * (currPoint->y - nextPoint->y)) ==
                yFlag1)
            {
    ...
    while (count < maxCount)
    {
        ...
        if (result->points.empty())
        {
            intersectionIndex = other->intersectWithSegment(
                points[nextIndex], points[(nextIndex + 1) % nbPoints()],
                intersectionPoint);
        ...
        {
            intersectionIndex = other->intersectWithSegment(
                result->points.back(), current->points[nextIndex],
                intersectionPoint, indexOfSegmentToIgnore);
            ...
            {
                // Intersection
                indexOfSegmentToIgnore =
                    (nextIndex + current->nbPoints() - 1) % current->nbPoints();
...
void PolygonPrimitive::toMesh(const char *filename)
{
    ...
    while (originalIndices.size() > 2)
    {
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            {
                fprintf(f, "f %d %d %d\n", originalIndices[i] + 1,
                        originalIndices[j] + 1, originalIndices[k] + 1);
...
PolygonPrimitive *PolygonPrimitive::cleanSelfIntersection(int startingIndex)
{
    ...
    for (int i = 0; i < size - 1; ++i)
    {
        ...
        {
            if (PointPrimitive::intersectSegments(
                    points[i], points[i + 1], points[j], points[(j + 1) % size],
                    intersection))
            {
...
int PolygonPrimitive::intersectWithSegment(Point3d &segmentStart,
                                           Point3d &segmentEnd,
                                           Point3d &intersection,
                                           int indexOfSegmentToIgnore)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (PointPrimitive::intersectSegments(segmentStart, segmentEnd,
                                              points[i], points[(i + 1) % size],
                                              tempIntersection))
        {
...
PolygonPrimitive *CurvePrimitive::makeVolumetric(double thickness)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        const Point3d &nextPoint =
            (i == size - 1) ? points[size - 1] : points[i + 1];

        Vector3d offsetVec(nextPoint.y - prevPoint.y,
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (dist > 2 * thickness)
        {
            fprintf(f, "   CDP_Sphere %lf %lf %lf %lf\n", points[i].z,
                    points[i].y, points[i].x, thickness);
            dist = 0;
        }
        dist += edge.length();
    }
